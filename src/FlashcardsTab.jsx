...
export default function FlashcardsTab() {
           ...
           d.id !== deck.id
             ...
             : {
                 ...
                 cards: d.cards.map(c =>
                   c.id !== card.id
                     ? c
                     : { ...c, question: formQ, answer: formA, image: newImg, taxonomy: 'Manual'}
                 ),
               }
         )
       );
              ...
              <div className="bg-white p-4 rounded shadow space-y-2 border border-gray-200 mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-gray-600">
                    {newFileObj?.name ?? (newImgUrl ? 'Using current image' : 'No image')}
                  </span>
                </div>
                <input
                  ref={newFileRef}
                ...
                {newImgUrl && (
                  <img src={newImgUrl} className="w-24 h-24 object-cover rounded border" />
                )}
  ...
  return (
    <div
      ...
      <button
        onClick={() =>
          isEditing ? finishEdit(card) : startEdit(card)
        }
      ...
      {isEditing ? (
        <>
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <span className="text-sm text-gray-600">
              {fileObj ? fileObj.name : formImg ? 'Using current image' : 'No image'}
            </span>
          </div>
          <input
            ref={inputFileRef}
   ...
   <div className={`flex items-center ${card.image ? 'gap-5' : 'gap-0'}`}>
     {card.image && (
       <img
         src={card.image}
         alt=""
     ...
     <div>
       <h2 className="font-semibold mb-1 text-black">
         Q: {card.question}
         <span className={`taxonomy-label ${getTaxonomyLabelColor(card.taxonomy)} ml-2`}>
                {card.taxonomy}
          </span>
       </h2>
