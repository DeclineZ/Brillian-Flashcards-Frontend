...
export default function QuizPlayer() {
  ...
  const evaluate = async () => {
    ...
    try {
      ...
      if (
        ...
        (record.percent === prevBest.percent &&
          record.time < (prevBest.time ?? Infinity))
      ) {
  ...
  return (
    <div className="flex flex-col items-center w-full min-h-screen bg-gray-50">
        ...
        {result && (
          ...
          >
            ...
            <p className="text-green-700">
              ✅ What You Did Well: {result.positive.join(", ")}
            </p>
            <p className="text-red-600">
              ❌ What You Missed: {result.negative.join(", ")}
            </p>
      ...
      {showXpMessage && <div className="xp-message">{xpMessage}</div>}
    </div>
  );
}
